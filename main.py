    ...
    try:
        if diagram_type == "triangle":
            ...
            if not isinstance(raw_points, list) or len(raw_points) != 2:
                raise ValueError(
                    "Coordinate-line diagrams require exactly two points."
                )
            first, second = (
...
async def send_question_message(
    ...
    if "geometry" not in question.tags:
...
def load_registered_user_ids() -> set[int]:
    """Load the persisted user ID set from users.json."""
    try:
        raw_users = json.loads(USERS_FILE.read_text(encoding="utf-8"))
    except FileNotFoundError as error:
        raise RuntimeError(f"User registry not found: {USERS_FILE}") from error
    except json.JSONDecodeError as error:
        raise RuntimeError(f"User registry is not valid JSON: {USERS_FILE}") from error

    if not isinstance(raw_users, list):
        raise RuntimeError("users.json must contain a JSON array of Telegram user IDs.")

    if any(
        not isinstance(user_id, int) or isinstance(user_id, bool)
        for user_id in raw_users
    ):
        raise RuntimeError("Every entry in users.json must be an integer user ID.")

    return set(raw_users)


def save_registered_user_ids() -> None:
    """Persist the user ID set as a stable, readable JSON array."""
...
def load_error_bank() -> dict[str, list[dict[str, object]]]:
    """Load persistent incorrect-answer records grouped by Telegram user ID."""
    try:
        raw_errors = json.loads(ERRORS_FILE.read_text(encoding="utf-8"))
    except FileNotFoundError as error:
        raise RuntimeError(f"Error bank not found: {ERRORS_FILE}") from error
    except json.JSONDecodeError as error:
        raise RuntimeError(f"Error bank is not valid JSON: {ERRORS_FILE}") from error

    if not isinstance(raw_errors, dict):
        raise RuntimeError("errors.json must contain an object keyed by user ID.")

    for user_id, records in raw_errors.items():
        if not isinstance(user_id, str) or not isinstance(records, list):
            raise RuntimeError(
                "errors.json must map string user IDs to arrays of error records."
            )
        if not all(isinstance(record, dict) for record in records):
            raise RuntimeError("Every error record in errors.json must be an object.")

    return raw_errors


def save_error_bank() -> None:
    """Persist all incorrect-answer records in a readable JSON format."""
...
def record_user_error(
    ...
    if user is None:
        logger.warning("Could not persist an error without an effective Telegram user.")
        return None
...
def load_questions() -> dict[str, tuple[Question, ...]]:
    ...
    try:
        ...
        raise RuntimeError(f"Question bank not found: {QUESTIONS_FILE}") from error
    except json.JSONDecodeError as error:
        raise RuntimeError(
            f"Question bank is not valid JSON: {QUESTIONS_FILE}"
        ) from error
    ...
    for position, raw_question in enumerate(raw_questions, start=1):
        ...
        if (
            ...
            raise RuntimeError(f"Question {question_id} has invalid tags.")
        if diagram is not None and not isinstance(diagram, dict):
            raise RuntimeError(f"Question {question_id} has invalid diagram data.")
        if "geometry" in tags and diagram is None:
            raise RuntimeError(
                f"Geometry question {question_id} must include diagram data."
            )
    ...
    if any(not questions for questions in questions_by_category.values()):
        raise RuntimeError(
            "Each configured category must contain at least one question."
        )
...
def seen_question_ids(context: ContextTypes.DEFAULT_TYPE) -> set[str]:
    ...
    if not isinstance(seen, set):
        raise RuntimeError(f"{SEEN_IDS_KEY} must be a set for the active user.")
    return seen
...
def next_question(
    ...
    """Choose a random unseen question, resetting only after the full deck ends."""
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Show the practice menu and start a fresh personal deck."""
...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Show usage totals to the administrator only."""
    ...
    await update.effective_message.reply_text(
        ...
        f"Total unique users registered in users.json: "
        f"{len(REGISTERED_USER_IDS)}"
    )
...
def choose_exam_question_ids(
    ...
    """Randomly select unused question IDs for the user's current exam segment."""
    used_ids = context.user_data.setdefault("exam_used_question_ids", set())
    if not isinstance(used_ids, set):
        raise RuntimeError("exam_used_question_ids must be a set.")
...
def current_exam_question(context: ContextTypes.DEFAULT_TYPE) -> Question | None:
    """Return the question currently awaiting an answer in a mock exam."""
...
async def send_next_exam_question(message, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Send the current mock-exam question with exam-specific callbacks."""
...
async def answer_exam_question(
    ...
    """Evaluate an exam answer and immediately advance the exam sequence."""
    query = update.callback_query
    question = QUESTION_BY_ID.get(question_id)
    expected_question = current_exam_question(context)

    if question is None or expected_question is None or expected_question.question_id != question_id:
        await query.edit_message_text(
    ...
    if exam_type == "sat" and question_index >= SAT_MODULE_QUESTIONS:
        if module == 1:
            ...
            await query.message.reply_text(
                f"Module 1 complete. Starting {route}."
            )
...
async def stop_exam(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Stop an active mock exam, show its partial score, and return to the menu."""
    query = update.callback_query
    exam_type = context.user_data.get("exam_type")
...
def mistake_review_keyboard(index: int, total: int) -> InlineKeyboardMarkup:
    """Build navigation controls for one-by-one mistake review."""
    if index + 1 < total:
...
async def show_mistake_review(
    ...
    """Render the currently selected mistake in the exam review."""
    query = update.callback_query
...
async def start_mistake_review(
    ...
    """Start reviewing only the mistakes from the completed exam session."""
    mistakes = context.user_data.get("exam_incorrect_answers", [])
    if not isinstance(mistakes, list) or not mistakes:
...
async def finish_mistake_review(
    ...
    """End the one-by-one mistake review and return to the main menu."""
    query = update.callback_query
    context.user_data.clear()
...
# --------------------------------------------------
# SCORE CONVERSION LOGIC & EXAM ENGINE
# --------------------------------------------------


def get_sat_math_score(raw_score, total_questions=44):
    """Converts raw correct answers (out of 44) to official 200-800 SAT scale."""
...
def get_ielts_reading_band(raw_score):
    """Maps raw score out of 40 to official IELTS Band score (1.0 - 9.0)."""
    if raw_score >= 39:
        return 9.0
    if raw_score >= 37:
        return 8.5
...
def evaluate_sat_exam(module_1_score, module_2_score):
    """Determines overall SAT scale based on Module 1 adaptive routing."""
    total_raw = module_1_score + module_2_score
    if module_1_score >= 15:  # High performance -> Hard Module 2
        scaled_score = get_sat_math_score(total_raw, total_questions=44)
        route_taken = "Hard Module 2 (Max Scale: 800)"
    else:  # Low performance -> Standard Module 2 (Capped)
...
async def send_exam_results(
    ...
    review_markup = InlineKeyboardMarkup(
        [[InlineKeyboardButton("📝 Review My Mistakes", callback_data="review_mistakes")]]
    )
...
def build_application(token: str) -> Application:
    ...
    application.add_handler(
        CallbackQueryHandler(
            answer_question,
            pattern=(
                r"^(?:answer|exam_answer):|^exam_stop$|"
                r"^review_(?:mistakes|next|done)$"
            ),
        )
    )
    application.add_handler(
...
def main() -> None:
    ...
    if not token:
        raise RuntimeError(
            "BOT_TOKEN is not set. Add the Telegram bot token as a Replit Secret "
            "named BOT_TOKEN before starting the bot."
        )

    logger.info(
        "Starting Study Sprint Telegram bot with %d questions",
        TOTAL_QUESTIONS,
    )
